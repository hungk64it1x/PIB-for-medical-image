...
'''
Reference https://github.com/yskmt/pb ==> Poisson image blending
'''
...
def poisson_copy_paste(path_img_positive, path_mask_positive, path_img_target, path_mask_target, offset=(0, 0)):
    img_src = io.imread(path_img_positive).astype(np.float64)
    img = cv2.imread(path_img_positive)[:,:,::-1]
    mask = cv2.imread(path_mask_positive)[:,:,::-1]
    blend_image = cv2.bitwise_and(img, mask)
    h, w = img.shape[0: 2]
    mask_copy = mask.copy()
    
    translation_matrix = np.float32([ [1, 0, offset[1]], [0, 1, offset[0]]])   
    mask_translation = cv2.warpAffine(mask_copy, translation_matrix, (w, h))  
    
    img_mask = cv2.cvtColor(blend_image, cv2.COLOR_RGB2GRAY)
    
    img_target = io.imread(path_img_target)
    mask_target = io.imread(path_mask_target, 0)
    img_target = cv2.resize(img_target, (w, h))
    try:
        mask_target = cv2.resize(mask_target, (w, h))[:,:,0]
    except:
        mask_target = cv2.resize(mask_target, (w, h))
    ...
    res_mask = mask_target + mask_translation[:, :, 1]
    result = {
        'image': img_pro,
        'mask': res_mask
    }
